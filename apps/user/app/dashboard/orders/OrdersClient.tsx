/**
 * @file app/dashboard/orders/OrdersClient.tsx
 * @description Client Component for the Orders History page.
 * 
 * Renders the order history UI. Uses TanStack Query hydrated with initial data
...
import { getOrdersByUserIdAction } from '../../../lib/orders/actions'
import type { User } from '@supabase/supabase-js'
import type { GetOrdersResult } from '../../../lib/orders/service'

interface OrdersClientProps {
	user: User;
	initialResult: GetOrdersResult;
}

export default function OrdersClient({ user, initialResult }: OrdersClientProps) {
	const { data: result } = useQuery({
		queryKey: ['orders', user?.id],
		queryFn: async () => await getOrdersByUserIdAction(user?.id),
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
						...
						<div className="bg-white shadow-xl rounded-2xl p-6 border border-gray-100">
							...
							<p className="text-3xl font-bold text-green-600">
								{orders.filter((o: any) => (o.status as string)).length}
							</p>
						</div>
						...
						<div className="bg-white shadow-xl rounded-2xl p-6 border border-gray-100">
							...
							<p className="text-3xl font-bold text-gray-900">
								{formatAmount(
									orders
										.filter((o: any) => (o.status as string) === 'paid')
										.reduce((sum: number, o: any) => sum + (o.amount_paise || 0), 0),
									'INR'
								)}
							</p>
						</div>
