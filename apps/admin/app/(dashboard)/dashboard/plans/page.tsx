		...
		<div className="min-h-screen bg-slate-50 p-4 md:p-8">
			<div className="max-w-6xl mx-auto">
				<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
					<div>
						<h1 className="text-xl md:text-2xl font-bold text-slate-900">Subscription Plans</h1>
						<p className="text-sm text-slate-500">Configure tiers for your Razorpay integration</p>
					</div>
					<Link
						href="/dashboard/plans/add"
						className="bg-blue-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-blue-700 transition shadow-sm whitespace-nowrap"
					>
						+ Create New Plan
					</Link>
				</div>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
					{plans.map((plan) => (
						<div key={plan.id} className="bg-white border border-slate-200 rounded-xl p-4 md:p-6 shadow-sm flex flex-col">
							<div className="flex justify-between items-start mb-4">
								...
								<div className="text-right">
									<p className="text-xl md:text-2xl font-black text-slate-900">₹{plan.amount / 100}</p>
							...
							<div className="space-y-3 mt-4 flex-1">
								<div className="text-xs">
									...
									<code className="bg-slate-50 p-1.5 rounded block border border-slate-100 text-slate-600 break-all text-[10px] md:text-xs">
										{plan.razorpay_plan_id || 'Not Linked'}
									</code>
								</div>
							</div>

							<div className="mt-6 pt-4 border-t border-slate-50 flex gap-2 md:gap-3">
								<Link
									href={'/dashboard/plans/edit/' + plan.id}
									className="flex-1 text-center text-xs md:text-sm font-medium py-2 border border-slate-200 rounded hover:bg-slate-50 transition"
								>
									Edit
								</Link>
								<Link
									href={'/dashboard/plans/' + plan.id}
									className="flex-1 text-xs md:text-sm text-center font-medium py-2 border border-slate-200 rounded hover:bg-slate-50 transition"
								>
									View
								</Link>
					...
					{plans.length === 0 && (
						<div className="col-span-full bg-white border-2 border-dashed border-slate-200 rounded-xl p-8 md:p-12 text-center">
